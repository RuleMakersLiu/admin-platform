...
核心工作台是「需求开发」页（`/pipeline/development`，登录后默认落地）。一条需求走完
6 个阶段，每个需确认阶段都会暂停等你审核：

**需求分析 → 页面设计 → 前端预览 → API 契约 → 自动审查 → 总结报告**

1. **输入需求**：在左侧输入框写需求（例：「给零售商品列表增加一个商品 ID 的筛选项，
   是现有页面加字段」），可上传设计稿/截图作参考。
2. **匹配项目**：点「分析需求并匹配页面功能」→ 系统自动匹配已接入的前端项目 Skill；
   匹配到后按钮变成「确认页面功能并执行」。改造现有页面时会让你选择目标页面。
3. **逐阶段执行**：流水线跑完一个需确认阶段就暂停（状态 `waiting_confirm`）：
   - 点「确认 XXX，继续」推进下一阶段；
   - 或点「提交反馈并重新生成」带修改意见退回重做。
   - 需求分析、API 契约阶段会先经子智能体评审，不通过自动带反馈重生成（有限次）。
4. **人工介入**（自动重试耗尽时）：流水线进入 `needs_human`，弹出介入对话框——可在线
   代码编辑器直接改产物文件并保存，「通过并继续」或「带反馈重新生成」恢复。也可在
   「待人工介入」队列（`/pipeline/intervention`）集中处理。
5. **确认报告，完成**：审查阶段通过后生成总结报告，**报告需你确认**（点「确认报告，
   完成流水线」）才会标记完成——这是给你审阅交付总结的最后关卡。
6. **完成后**：可「下载前端代码」、「提交到 Git」（填 commit message 自动提交）；
   若验收后发现预览/契约/报告有问题，可在「验收后需要调整」里选阶段 + 写反馈，回到
   指定阶段重新生成。

> 阶段状态实时流式显示（执行日志 + 各阶段输出折叠面板）。失败会在产物区显示原因和
> 「重新生成当前阶段」。
